...
public class OSMWayMap {
    ...
    public OSMWayMap(int capacity, boolean useTestMethods) {
    ...
    Node get(long id) {
        int position = (Long.hashCode(id) & MASK) % (table.length - 1);
        for (
            Node n = table[position]; n != null; n = n.next) {
            if (n.id == id) {
                return n;
            }
        }
    ...
    private class Node extends OSMWay {
        private long id;
        private Node next;

        private Node(long id, OSMWay way, Node next) {
            addAll(way);
            this.id = id;
            this.next = next;
        }
