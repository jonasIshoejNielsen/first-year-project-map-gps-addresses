...
public class InteractionController {
    ...
    public List<SearchResult> addressSearch(String prefix) {
        return viewController.addressSearch(prefix);
    }
    ...
    public void goTo(OSMAddress address) {
        viewController.goTo(address);
    }

    public RouteDescription findRoute(OSMAddress from, OSMAddress to, RouteType routeType, boolean calculateSpeed, float routePannelRightX){
        int adresses = 1;
        float lon = from.getHousePlacement().getLon();
        float lat = from.getHousePlacement().getLat();
        if(to != null){
            adresses++;
            lon += to.getHousePlacement().getLon();
            lat += to.getHousePlacement().getLat();
        }
        ...
        viewController.setRoute(from, to);
        return modelController.findRoute(from, to, routeType, calculateSpeed);
    }
    ...
    public List<SearchResult> houseSearch(String src, String houseNumber) {
        return modelController.houseSearch(src, houseNumber);
    }
