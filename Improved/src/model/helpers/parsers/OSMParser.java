...
public class OSMParser {
    ...
    private boolean parse() throws XMLStreamException{
        ...
        thread = new Thread(() -> {
            ...
            mapProgres++;
            addresses = addressParser.getAddresses();
            postcodes = addressParser.getPostcodes();
            cities = addressParser.getCities();
        ...
        if(useTestMethods) {
            ...
            System.out.println("Number of adresses: " + addressParser.numberOfAddresses());
    ...
    private void buildKDtreeNumbers() {
        ...
        for(OSMAddress osmAddress: addresses.getValueSet()) {
            for(String postcode : osmAddress.getPostcodes()) {
                for (HousePlacement placement : osmAddress.getHousePlacements(postcode)) {
                    if (placement.getHouseNumber() == null) continue;
                    float lon = placement.getLon();
                    float lat = placement.getLat();

                    Rectangle2D rectangle2D = new Rectangle2D.Float(lon, lat, Float.MIN_VALUE, Float.MIN_VALUE);
