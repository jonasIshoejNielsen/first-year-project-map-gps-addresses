...
public class SelectedSearchPanel extends JComponent {
    ...
    public void updatePanel(OSMAddress newAddress, String postcode) {
        if(newAddress.getStreet() != null) street.setText(newAddress.getStreet());
        else street.setText("");
        cityAndZip.setText(postcode + ", " + newAddress.getCity());
        lonLat.setText("Longtitude: " + newAddress.getHousePlacement(postcode).getLon() / MathConstants.getLonFactor() + " Lattitude:" + newAddress.getHousePlacement(postcode).getLat()*-1);
    }
}
