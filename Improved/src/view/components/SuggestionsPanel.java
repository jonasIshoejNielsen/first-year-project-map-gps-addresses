...
public class SuggestionsPanel extends JComponent implements Serializable {
    ...
    private JButton getSearchButton(SearchResult result, boolean isFirst) {
        ...
        button.addActionListener(e -> {
            ...
            if(result.getId() == SearchID.ADDRESS) {
                this.updateButtons(result.getAddress(), result.getPostcode());
            }
        });
    ...
    private void updateButtons(OSMAddress address, String postcode) {
        List<SearchResult> results = new ArrayList<>();
        int max = (address.getHousePlacements(postcode).size() > 8) ? 8 : address.getHousePlacements(postcode).size();

        for(int i = 0; i < max; i++) {
            OSMAddress temp = new OSMAddress(address.getCity(), address.getHousePlacement(postcode, i),postcode, address.getStreet());
            SearchResult result = new SearchResult(temp,postcode, SearchID.HOUSENUMBER);
            results.add(result);
        }
        this.inputText = "";
