...
public class InteractionView extends JComponent{
    ...
    public void setDefaultSearch() {
        ...
        searchButton.addActionListener(e -> {
            if(resultList != null && resultList.size() > 0) {
                ...
                interactionController.goTo(address, resultList.get(0).getPostcode());
            } else {
    ...
    public DocumentListener inputDocumentListener(JTextField inputField, InputFieldType type) {
        return new DocumentListener() {
            ...
            private void change() {
                ...
                appendSuggestions(interactionController.findAddresses(input, 10), input, inputField, type);
            }
        };
    }
    ...
    public void setSelectedAddress(SearchResult result, JTextField inputField, InputFieldType type) {
        if(type == InputFieldType.DEFAULT) {
            ...
            interactionController.goTo(selectedAddress.getAddress(), result.getPostcode());
            selectedSearchPanel.updatePanel(selectedAddress.getAddress(), result.getPostcode());
