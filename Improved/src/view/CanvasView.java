...
public class CanvasView extends JComponent{
    ...
    private float minLat, minLon, maxLat, maxLon, nearestDist;
    private final float ZOOMFACTOR = 1.5f;

    private List<Point2D> route, visited;
    private Point2D mouseModelCoords;
    ...
    public void paint(Graphics _g){
        ...
        viewRect = new Rectangle2D.Float(-OFFSET, -OFFSET, getWidth()+OFFSET*2, getHeight()+OFFSET*2);
        ...
        if(showNearestRoad && nearestRoad != null) {
            ...
            rect.setRect(mouseModelCoords.getX(), mouseModelCoords.getY(),0.0001f,0.0001f );
            g.draw(rect);
            drawString(nearestRoad.getName(), nearestRoad.getMedian());
        }
        if(selectedAddress != null) {
            Point2D p = new Point2D.Float(selectedAddress.getHousePlacement(selectedAddressPostcode).getLon(), selectedAddress.getHousePlacement(selectedAddressPostcode).getLat());
    ...
    private void printHouseNumbers() {
        if(kdTreeHouseNumbers == null) return;
        for(EnhancedShape enhancedShape : kdTreeHouseNumbers.rangeSearch(viewRect)) {
    ...
    private  void drawString(String string, Point2D point2D){
        ...
        if(!viewRect.intersects(point2D.getX(), point2D.getY(), Float.MIN_VALUE,Float.MIN_VALUE)) return;
    ...
    private void outLineKDTree(Graphics2D g) {
        ...
        path.moveTo(viewRect.getMinX(),viewRect.getMinY());
        path.lineTo(viewRect.getMinX(),viewRect.getMaxY());
        path.lineTo(viewRect.getMaxX(),viewRect.getMaxY());
        path.lineTo(viewRect.getMaxX(),viewRect.getMinY());
        path.lineTo(viewRect.getMinX(),viewRect.getMinY());
    ...
    private void fillArea(OSMType type){
        ...
        List<EnhancedShape> shapes = (type.equals(OSMType.COASTLINE)) ? canvasModel.getCoastline(viewRect) : canvasModel.getTree(type).rangeSearch(viewRect);
    ...
    private void drawLine(OSMType type){
        ...
        for (EnhancedShape line: canvasModel.getTree(type).rangeSearch(viewRect)) {
    ...
    public void drawSelectedAddress(OSMAddress address, String selectedAddressPostcode){
        if(address == null) return;
        if(address.equals(this.selectedAddress)) return;
        this.selectedAddress = address;
        this.selectedAddressPostcode = selectedAddressPostcode;
    }
